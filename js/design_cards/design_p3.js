function design_p3_redesign(nCards) {
    let place_div = document.getElementById('area_player3_cards');
    ...
    const idealH = 0.73*0.2*0.9*screen.availHeight*0.90;
    if (baseH > idealH) {
        baseH = idealH;
    }
    place_div.classList = '';
    ...
    place_div.innerHTML = design_p3_cards_html(nCards, baseH);
    setTimeout(()=> { design_p3_recheck_size(place_div, baseH, idealH, 1) }, 300);
}

function design_p3_recheck_size(place_div, baseH, idealH, passo) {
...
function design_p3_card_groups_recalc_height(place_div, baseH) {
